    ...
    <section id="universitiesSection" className="container">
      <div className="universitiesContainer shapeParent my-5">
        ...
        <div className="mt-5 d-flex justify-content-end">
          {pageIndex > 0 ? (
            <button onClick={() => setPageIndex(pageIndex - 12)} className="btn btn-primary rounded-pill me-2" style={{width:"100px"}}>
              Previous
            </button>
          ) : (
            ""
          )}
          <button onClick={() => setPageIndex(pageIndex + 12)} className="btn btn-success rounded-pill" style={{width:"100px"}}>Next</button>
        </div>
      </div>
    </section>
  );
